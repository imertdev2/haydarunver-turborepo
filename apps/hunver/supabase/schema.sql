...
-- 5. İşletme rehberi (Çıralı / Olympos / Adrasan / Kemer)
create table if not exists businesses (
  id uuid default gen_random_uuid() primary key,
  name text not null,
  slug text unique,
  category text not null,             -- 'konaklama', 'restoran', 'tekne_turu', 'dalis', 'rehber', 'macera', 'wellness', 'ulasim', 'diger'
  region text not null,               -- 'cirali', 'olympos', 'adrasan', 'kemer'
  phone text,
  whatsapp text,
  website text,
  instagram text,
...
-- Herkes aktif işletmeleri görebilir; yazma yalnızca service_role
create policy "Herkes aktif isletmeleri gorebilir" on businesses
