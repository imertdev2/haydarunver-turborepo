...
import { ChevronLeft, ChevronRight, Check, Loader2, Calendar } from "lucide-react"
            ...
            <div className="flex flex-col items-center gap-3 sm:flex-row sm:justify-center sm:flex-wrap">
              ...
              <button
                onClick={() => {
                  if (!selectedService || !selectedDate || !selectedSlot) return
                  const ics = buildIcs({
                    uid: `${appointmentId ?? Date.now()}@hunver`,
                    title: `${selectedService.name} — Haydar Ünver`,
                    description: form.note
                      ? `Not: ${form.note}`
                      : "Haydar Ünver ile randevu",
                    date: selectedDate,
                    startTime: selectedSlot.start,
                    endTime: selectedSlot.end,
                    organizerName: "Haydar Ünver",
                    attendeeName: form.name,
                    attendeeEmail: form.email || undefined,
                  })
                  const blob = new Blob([ics], { type: "text/calendar;charset=utf-8" })
                  const url = URL.createObjectURL(blob)
                  const a = document.createElement("a")
                  a.href = url
                  a.download = `randevu-${selectedDate}.ics`
                  document.body.appendChild(a)
                  a.click()
                  document.body.removeChild(a)
                  URL.revokeObjectURL(url)
                }}
                className="inline-flex items-center gap-2 rounded-lg border border-[#C8A96A]/30 bg-[#C8A96A]/[0.06] px-5 py-2.5 text-sm font-semibold text-[#C8A96A] transition-all hover:bg-[#C8A96A]/15"
              >
