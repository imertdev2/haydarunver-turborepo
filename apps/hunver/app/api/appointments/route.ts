...
import { sendAdminNewAppointmentEmail } from "@/lib/email"
...
export async function POST(request: Request) {
  ...
  // Admin'e bildirim — mail başarısızlığı randevuyu engellemesin
  const serviceName = (data.services as { name: string } | null)?.name ?? "Hizmet"
  try {
    const ics = buildIcs({
      uid: `${data.id}@hunver`,
    ...
    console.error("[appointments] admin email failed:", e)
  }
